...
int StudentWorld::move()
{
    ...
    for (int i=0; i<actors.size();i++) {
        if (!actors[i]->isAlive()) {
            vector<Actor*>::iterator it = actors.begin() + i;
            delete actors[i];
            actors.erase(it);
        }
    }
    ...
    for (vector<Actor*>::iterator it=actors.begin(); it!=actors.end(); it++) {
        if ((*it)->isAlive() && (*it)->isDamageable()) {
            if ((*it)->getX()==x && (*it)->getY()==y) {
                (*it)->loseHealth();
                if (((*it)->getHealth())<=0) {
                    //the following will only be applied to dead thiefbots
                    if (((*it)->canSteal()) && ((*it)->getStealingStatus())) {
                        returnAGoodie((*it)->getX(),(*it)->getY(),(*it)->getGoodieType());
                    }
                }
                return true;
            }
...
bool StudentWorld::equalPos(int x1, int y1, int x2, int y2) {
    if (x1==x2 && y1==y2) { return true; }
    return false;
}

bool StudentWorld::overlaps(Actor* actor1, Actor* actor2) {
    return equalPos(actor1->getX(),actor1->getY(),actor2->getX(),actor2->getY());
}

bool StudentWorld::playerIsStandingOn(Actor* actor) {
    if (m_avatar->isAlive() && overlaps(actor,m_avatar)) {
        return true;
    }
    return false;
}
...
//bool StudentWorld::blocksMarble(int x, int y) {
//    if (getActor(x,y) == nullptr) { return false; }
//    if (getActor(x,y)->allowsMarble()) {
//        return false;
//    }
//    return true;
//}
...
Actor* StudentWorld::getMarble(int x, int y) {
    for (vector<Actor*>::iterator it=actors.begin(); it!=actors.end(); it++) {
        if ((*it)->isAlive() && (*it)->isSwallowable()) {
            if ((*it)->getX()==x && (*it)->getY()==y) {
                return *it;
...
bool StudentWorld::killsPea(int x, int y) {
    for (vector<Actor*>::iterator it=actors.begin(); it!=actors.end(); it++) {
        ...
        if (actors[i]->countsForFactoryCount()) {
            for (int xi=xLeftMax; xi<=xRightMax; xi++) {
                for (int yi=yDownMax; yi<=yUpMax; yi++) {
                    if (actors[i]->getX()==xi && actors[i]->getY()==yi) {
                            count++;
                    }
                }
            }
...
bool StudentWorld::isGoodie(int x, int y, int& goodieType, int& goodieX, int& goodieY) {
    for (int i=0; i<actors.size(); i++) {
        if (actors[i]->isStealable()) {
            if ((actors[i]->getX()==x) && (actors[i]->getY()==y)) {
                goodieX = actors[i]->getX();
                goodieY = actors[i]->getY();
                if (actors[i]->givesAmmo()) {
                    goodieType = 10;
                }
                if (actors[i]->givesLife()) {
                    goodieType = 20;
                }
                if (actors[i]->givesHealth()) {
                    goodieType = 30;
                }
                return true;
            }
...
Actor* StudentWorld::getActorAt(int x, int y) {
    if ((x==getPlayersX()) && (y==getPlayersY())) {
        return m_avatar;
    }
    for (int i=0; i<actors.size(); i++) {
        if ((actors[i]->getX()==x) && (actors[i]->getY()==y)) {
...
bool StudentWorld::foundTargetInSight(int x, int y, int dir) {
    Actor* tempActor;
    switch (dir) {
        case 0: //right
            for (int i=1; i<(16-x); i++) {
                ...
                if (tempActor == m_avatar) {
                    return true;
                }
                if (tempActor->blocksBotRange()) {
                    return false;
                }
            }
            break;
